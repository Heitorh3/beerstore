...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleNotValidException(MethodArgumentNotValidException exception, Locale locale){
        ...
        Stream<ObjectError> erros = exception.getBindingResult().getAllErrors().stream();

        List<ApiError> apiErrors = erros
    ...
    public ResponseEntity<ErrorResponse> handleInvalidFormatException(InvalidFormatException exception, Locale locale){
        ...
        final String erroCode = "generic-1";
        final HttpStatus status = HttpStatus.BAD_REQUEST;
        final ErrorResponse errorResponse = ErrorResponse.of(status, toApiError(erroCode, locale, exception.getValue()));
    ...
    public ResponseEntity<ErrorResponse> handleBusinessException(BusinessException exception, Locale locale){
        ...
        final String erroCode = exception.getCode();
        final HttpStatus status = exception.getStatus();

        final ErrorResponse errorResponse = ErrorResponse.of(status, toApiError(erroCode, locale));
    ...
    private ApiError toApiError(String code, Locale locale, Object... args){
        ...
        try{
            ...
            LOG.error("Cold not find any message for {} code under {} locale", code, locale);
